...
import { createScene, tileToWorld } from './testScene3d'
import { createRobot, getDirRotation, animateRobot, setRobotAnim } from './testRobotModel'
import { createGear, createLamp, createTarget, animateObjects, collectGear, lightUpLamp } from './testObjects3d'
import './TestRobot.css'
...
export default function RobotRenderer({ levels, narrative }) {
  ...
  const [overParUsed, setOverParUsed] = useState(false)
  ...
  return (
    <div className="rb-wrapper">
      ...
      <div className="rb-level-info">
        <div className="rb-level-num">LEVEL {levelIndex + 1}</div>
        <div className="rb-level-par">PAR {level.par}</div>
        {phase === 'running' && <div className="rb-level-steps">STEP {execIndex + 1}</div>}
        {overParUsed && <div className="rb-level-warning">NO MORE RETRIES</div>}
      </div>
        ...
        if (isGameOver || failed) {
          return (
            <div className="rb-result-overlay">
              ...
              <div className="rb-result-info">
                {isGameOver ? `${flatCount} STEPS · PAR ${level.par}` : 'GAME OVER'}
              </div>
              <button className="rb-btn" onClick={() => {
                setLevelIndex(0); setOverParUsed(false); setGears([]); setPhase('intro')
              }}>RESTART</button>
            </div>
          )
        }

        if (won && isOverPar && !overParUsed) {
          return (
            <div className="rb-result-overlay">
              <div className="rb-result-title">OVER PAR</div>
              <div className="rb-result-info">{flatCount} STEPS · PAR {level.par}</div>
              <div className="rb-result-suboptimal">
                YOU GET ONE RETRY. NEXT TIME OVER PAR IS GAME OVER.
              </div>
              <button className="rb-btn" onClick={() => { setOverParUsed(true); retry() }}>RETRY</button>
            </div>
          )
        }
